...
function changeStatus(eve){
    ...
    return false;//阻止默认行为的发生,此例中是放在超链接上的操作,不取消想想会是什么结果
}
...
function deleteMuliRecord(eve) {
    ...
    ids = ids.substr(0, ids.length - 1);//去掉最后的一个分隔符 -
    names = names.substr(0, names.length - 1);//去掉最后的一个分隔符 ,
    //首先判断是否选择了数据
...
//跳转到指定页码,这样封装的方式目的在于能够方便的在多个函数之间调用,简化页面的跳转操作
function gotoPage(pageNum, pageSize) {
    pageNum = pageNum == null ? 1 : pageNum;
    pageSize = pageSize == null ? 10 : pageSize;
    let url = $("#searchForm").attr("action");//获取搜索框中的搜索路由地址
    $.ajax({
        type: "GET",
