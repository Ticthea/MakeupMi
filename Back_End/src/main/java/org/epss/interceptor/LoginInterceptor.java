...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (
                ...
                || uri.indexOf("/user/loginCheck") >= 0
                || uri.indexOf("/businessrest/loginCheck") >=0
        ) {
            return true;//放行
        }
        ...
        //对于商户的放行策略
        String bname = (String) session.getAttribute("BUSINESS_SESSION");
        if(bname != null && bname != ""){
            return true;
        }
