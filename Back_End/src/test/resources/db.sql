...
BEGIN
    ...
    WHILE
        i < 100
        DO
            insert into userinfo (uiphone,uiaddress) value (concat('13', FlOOR(RAND() * 899999999 + 100000000)), '大黑山程序员养殖基地');
            SET i = i + 1;
...
(
    ...
    bavatar         varchar(500)          default null comment '头像',
...
BEGIN
    ...
    WHILE
        i < 100
        DO
            insert into business (bname, bpass, bdes, bdelivery, bmark, bavatar, baddress, brecommendation,
                                  bstatus) value
                (concat('business', i), 'fe8fb64eb34764f9e0a58a32a819dcb2', concat('business', i, '的介绍'), '15起送',
                 FlOOR(RAND() * 5 + 1), '/upload/bussiness.png',
...
(
    ...
    tavatar  varchar(500)          default null comment '类别图片',
    add_time timestamp   not null default current_timestamp comment '创建时间',
    up_time  timestamp   not null default current_timestamp on update current_timestamp comment '修改时间',
    primary key (`tid`)
) auto_increment = 1
  charset = utf8 comment '商品类别表';
commit;
...
(
    ...
    gavatar  varchar(500)          default null comment '商品图片',
    fbid     integer     not null comment '商户信息',
    ...
    WHILE
        ...
        DO
            insert into goods (gname, gdes, gprice, gavatar, fbid, ftid) value
                (CONCAT('商品', i), CONCAT('商品介绍', i), RAND() * 15 + 5, '/upload/goods.png', FlOOR(RAND() * 99 + 1),
                 FlOOR(RAND() * 9 + 1));
            SET i = i + 1;
...
(
    ...
    cavatar  varchar(500)          default null comment '头像',
    ...
    WHILE
        ...
        DO
            insert into customer (cname, cphone, cemail, cpass, caddress, cavatar, cgender, cstatus)
                value (concat('cus', i), concat('138', FlOOR(RAND() * 80000000 + 10000000)),
                       concat('138', FlOOR(RAND() * 80000000 + 10000000), '@epss.vip'),
                       '696aa7bb5ee7ac9135f7ed4ef526fe4d', concat('收货地址', i), '/upload/customer.png', false, false);
            SET i = i + 1;
    ...
    WHILE
        ...
        DO
            insert into orders (onumber, ostatus, fgid, fcid, fbid) value
                (FlOOR(RAND() * 9 + 1), FlOOR(RAND() * 3 + 1), FlOOR(RAND() * 99 + 1), FlOOR(RAND() * 99 + 1),
                 FlOOR(RAND() * 99 + 1));
            SET i = i + 1;
